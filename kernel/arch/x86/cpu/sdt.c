...
int byte_checksum(char *p, size_t s)
{
	int8_t ret = 0;
...
struct acpi_rsdp *x86_get_acpi_rsdp()
{
...
struct acpi_rsdt *x86_get_acpi_rsdt()
{
	if(_rsdt) return _rsdt;

	struct acpi_rsdp *rsdp = x86_get_acpi_rsdp();
	if(!rsdp) return NULL;
...
struct acpi_madt *x86_get_acpi_madt()
{
	if(_madt) return _madt;

	struct acpi_rsdt *rsdt = x86_get_acpi_rsdt();
	if(!rsdt) return NULL;
...
int x86_get_cpu_count()
{
	...
	struct acpi_madt *madt = x86_get_acpi_madt();
